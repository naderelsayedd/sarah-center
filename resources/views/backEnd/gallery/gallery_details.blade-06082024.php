                ...
                <div class="white-box">
                  ...
                  <div class="row">
                    @foreach($gallery->images as $gallery_image)
                        <div class="col-md-4 image-container fixed-size position-relative">
                            <img src="{{ asset($gallery_image->images) }}" class="img-thumbnail img-fit" data-toggle="modal" data-target="#carouselModal" data-slide-to="{{ $loop->index }}" data-gallery-id="{{ $gallery_image->id }}" data-image-path="{{ asset($gallery_image->images) }}" data-image-id="{{ $gallery_image->id }}">
                            <div class="comment-count-badge position-absolute" style="top: 0px;left: 10px;background-color: #e3e0e0;border-radius: 50%;padding: 1px 10px;font-size: 12px;font-weight: bold;color: #333;">
                            ...
                            <div class="image-overlay">
                                <?php if (Auth::user()->role_id == 4):?>
                                    <form action="{{ route('deleteSingleImage', $gallery_image->id) }}" method="POST" class="delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="delete-icon btn btn-link p-0" style="margin-right: 22px;">
                                            <i class="fas fa-trash"></i>
                                        </button>
                                    </form>
                                <?php endif?>
                                <a href="{{ route('downloadSingleImage', $gallery_image->id) }}" class="download-icon btn btn-link p-0">
                                    <i class="fas fa-download" style="color: black;background: white;"></i>
                                </a>
                            </div>
                        </div>
                    @endforeach
      ...
      repliesHtml += `
        <div class="media comment" id="comment-${reply.id}">
          ...
          <div class="media-body">
            ...
            <strong>${reply.comment}</strong><br>
