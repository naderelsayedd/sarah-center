            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                select: function(start, end, allDays) {
                    ...
                    $('#saveButtonForAddEvent').click(function() {
                        ...
                        var data_type = 'ajax';
                        var upload_file_name = $('#upload_file_name')[0].files[0];
                        var formData = new FormData();
                        formData.append('event_title', event_title);
                        formData.append('event_location', event_location);
                        formData.append('event_des', event_des);
                        formData.append('url', url);
                        formData.append('from_date', from_date);
                        formData.append('to_date', to_date);
                        formData.append('data_type', data_type);

                        // Append each role id to the FormData
                        if (role_ids) {
                            for (var i = 0; i < role_ids.length; i++) {
                                formData.append('role_ids[]', role_ids[i]);
                            }
                        }

                        if (upload_file_name) {
                            formData.append('upload_file_name', upload_file_name);
                        }
                ...
                eventClick: function(event, jsEvent, view) {
                    ...
                    if (event.event.extendedProps.type == 'admission_query') {
                        ...
                        if (event.event.extendedProps.image) {
                            ...
                            $('#eventFile').attr('href', base_system_url + '/' + event.event.extendedProps.image);
                        }
