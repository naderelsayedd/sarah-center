(function($) {
	...
	validateAddQuestionnaireForm = () => {
        var questionnaire_title = $("#questionnaire_title").val();
	...
	validateUpdateQuestionnaireForm = () => {
        var questionnaire_title = $("#questionnaire_title_update").val();
        if (questionnaire_title === "") {
            $(".modal_input_validation").show();
            $(".modal_input_validation").html(
                "<font style='color:red;'>Must be Fill Up</font>"
	...
	addRowLessonPop = () => {
        $("#addRowBtn").button("loading");
        var tableLength = $("#productTablePopup tbody tr").length;
        var url = $("#url").val();
        var tableRow;
        var arrayNumber;
        var count;
        if (tableLength > 0) {
            tableRow = $("#productTablePopup tbody tr:last").attr("id");
            arrayNumber = $("#productTablePopup tbody tr:last").attr("class");
            count = tableRow.substring(3);
            count = Number(count) + 1;
            arrayNumber = Number(arrayNumber) + 1;
        } else {
        ...
        if (tableLength > 0) {
            $("#productTablePopup tbody tr:last").after(newRow);
        } else {
            $("#productTablePopup tbody").append(newRow);
        }
        $(".common-select").addClass("new_select_css");
    };
    ...
    addRowInSell = () => {
        ...
        $.ajax({
            ...
            success: function(response) {
				...
				"</div></td>" +
				'<td class="border-top-0" width=""><div class="primary_input">' +
				'<input type="text" name="descriptionItem[]" id="descriptionItem' +
				count +
				...
				"</div></td>" +
				'<td class="border-top-0" width=""><div class="primary_input">' +
				'<input type="text" name="discount[]" onkeyup="getTotalByPrice(' +
				count +
				')" id="discount' +
				count +
				'"  autocomplete="off" class="primary_input_field"  />' +
				...
				count +
				'"  autocomplete="off" value="15" class="primary_input_field" readonly  />' +
				'<span class="focus-border"></span>' +
				"</div></td>" +
				'<td class="border-top-0"><div class="primary_input">' +
				'<input type="text" name="total[]" id="total' +
    ...
    getTotalByPrice = (row = null) => {
        if (row) {
            ...
            if (product_id > 0) {
				var discount = Number($("#discount" + row).val());
				if(discount) {
					var total =
                    (Number($("#unit_price" + row).val()) *
					Number($("#quantity" + row).val())) - (Number(discount));
					if($("#vat").length > 0) {
						var vat = Number($("#vat" + row).val());
						vat = (Number(total) * Number(vat)) / 100; 
	...
	// for upload specilized_certificate Document  in Staff Add Module
    var specilized_certificate_documentfileInput = document.getElementById("specilized_certificate");
    if (specilized_certificate_documentfileInput) {
        specilized_certificate_documentfileInput.addEventListener("change", showFileName);
		
        function showFileName(event) {
            var specilized_certificate_documentfileInput = event.srcElement;
            var fileName = specilized_certificate_documentfileInput.files[0].name;
            document.getElementById(
                "placeholderSpecilized"
			).placeholder = fileName;
		}
	}
	
	// for upload specilized_certificate Document  in Staff Add Module
    var certificate_documentfileInput = document.getElementById("certificate");
    if (certificate_documentfileInput) {
        certificate_documentfileInput.addEventListener("change", showFileName);
		
        function showFileName(event) {
            var certificate_documentfileInput = event.srcElement;
            var fileName = certificate_documentfileInput.files[0].name;
            document.getElementById(
    ...
    $(document).on("click", "#removeMark", function(event) {
	 ...
	 // Assign class routine get subject
    $(document).on("click", "#removeMarkPop", function(event) {
        $(this).closest("tr").remove();
	});
...
addRowLessonPop = () => {
        ...
        newRow += "<td  class=''>";
        newRow +=
            "<button class='primary-btn icon-only fix-gr-bg' type='button' id='removeMarkPop'>";
        newRow += "<span class='ti-trash'></span>";
        newRow += "</button>";
