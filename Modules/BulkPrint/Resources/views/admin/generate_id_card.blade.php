        ...
        <div class="row">
            ...
            <div class="white-box">
                <div class="row">
                            ...
                            <div class="col-lg-4 mt-30-md" id="student-card-div">
                                <label class="primary_input_label" for="">@lang('common.student_list')  <span class="text-danger"> *</span></label>
                                <select class="primary_select  form-control{{ $errors->has('student_list') ? ' is-invalid' : '' }}" id="student_list" name="student_id">
                                   
                                    <option data-display=" @lang('common.student_list') *" value=""> @lang('common.student_list') *</option>
                                  
                                </select>
                                <div class="pull-right loader loader_style" id="student_list">
    ...
    $("#role_id").on("change", function() {
        ...
        $.ajax({
            ...
            success: function(data) {            
                $.each(data, function(i, item) {
                    if (item.length) {
                        $("#student_list").find("option").not(":first").remove();
                        $("#student-card-div ul").find("li").not(":first").remove();

                        $.each(item, function(i, studentList) {
                            $("#student_list").append(
                                $("<option>", {
                                    value: studentList.id,
                                    text: studentList.title,
                                })
                            );

                            $("#student-card-div ul").append(
                                "<li data-value='" +
                                studentList.id +
                                "' class='option'>" +
                                studentList.title +
                        ...
                        $("#student-card-div .current").html("ID Card *");
                        $("#student_list").find("option").not(":first").remove();
                        $("#student-card-div ul").find("li").not(":first").remove();
                    }
                });
