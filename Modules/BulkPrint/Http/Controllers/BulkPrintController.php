...
class BulkPrintController extends Controller
{
    ...
    public function studentidBulkPrintSearch(Request $request){
        ...
        if($request->role==2){
            ...
            $s_students = $s_students->status()->where('id',$request->student_id)->get();

        }elseif($request->role==3){
            $studentGuardian = SmStudent::where('school_id', Auth::user()->school_id)
    ...
    public function studentList(Request $request){
        try {
            ...
            foreach($students as $students_list){
                ...
                $d['id']=$students_list->id;
                $d['title']=$students_list->first_name;
                $studentList[]=$d;
            }
    ...
    public function certificateBulkPrint(){
        try {
            ...
            $students = SmStudent::get();
            return view('bulkprint::admin.generate_certificate_bulk', compact('roles','classes', 'certificates','students'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function certificateBulkPrintSearch(Request $request)
    {
        try {
            ...
            $data['students'] = SmStudent::where('id',$request->student_id)->get();
