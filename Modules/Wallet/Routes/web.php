...
Route::prefix('wallet')->middleware(['auth', 'subdomain'])->group(function() {
    ...
    Route::post('get-session-id', [WalletController::class, 'getSessionId'])->name('wallet.get-session-id');
    Route::post('make-payment', [WalletController::class, 'makePayment'])->name('wallet.make-payment');
    Route::post('make-payment-gpay', [WalletController::class, 'makePaymentGpay'])->name('wallet.make-payment-gpay');
});
