...
use Modules\Wallet\Http\Helper\WalletHelper;
...
class WalletController extends Controller
{
    ...
    {
        $session_data = WalletHelper::getSessionId($request->all());
        return $session_data;
    }

    public function makePayment(Request $request)
    {
        $result = WalletHelper::makePayment($request->all());
        if (isset($result['IsSuccess']) && $result['IsSuccess'] === true) {

            /*add wallet transaction*/
            $addPayment = new WalletTransaction();
            $addPayment->amount= $result['request']['Amount'];
            $addPayment->payment_method= 'Card';
            ...
            $data['amount'] = $result['request']['Amount'];
    ...
    public function makePaymentGpay(Request $request)
    {
        $result = WalletHelper::exicutePaymentGpay($request->all());
        if (isset($result['IsSuccess']) && $result['IsSuccess'] === true) {
            /*add wallet transaction*/
            $addPayment = new WalletTransaction();
            $addPayment->amount= $result['request']['InvoiceValue'];
            $addPayment->payment_method= 'Card';
            $addPayment->user_id= Auth::user()->id;
            $addPayment->type= 'diposit';
            $addPayment->status = 'approve';
            ...
            // Notification Start
            $data['title'] = "Wallet Amount Add";
            $data['amount'] = $result['request']['InvoiceValue'];
            $this->sent_notifications('Wallet_Add', (array)auth()->user()->id, $data, ['Student', 'Parent']);
