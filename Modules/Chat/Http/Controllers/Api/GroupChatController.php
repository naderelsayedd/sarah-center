...
class GroupChatController extends Controller
{
    ...
    public function show(Group $group)
    {
        ...
        // $group->load('users');
        if (!GroupUser::where('user_id', auth()->id())->where('group_id', $group->id)->first()) {
    ...
    public function destroy(Request $request)
    {
        ...
        if ($relatedGroup->role != 1) {
            return response()->json(['status' => 'failed','notPermitted' => true,'message' => 'Only admin role can delete the group.']);
        }
            ...
            foreach ($group->users as $user) {
                if ($user->id != auth()->id()) {
                    try {
                        ...
                        // Log the error or do something else if you want
                        // Log::error("Error notifying user {$user->id}: {$e->getMessage()}");
                    }
                }
            }
    ...
    {
        $group = Group::find($request->group_id);
        $already_added = GroupUser::where(['user_id'=>$request->user_id,'group_id'=>$request->group_id])->count();
        if ($already_added>0) {
            return response()->json(['status' => 'failed', 'message' => 'User already added.']);
        }else{
            GroupUser::create([
                'group_id' => $group->id,
                'user_id' => $request->user_id,
    ...
    {
        $user = GroupUser::where('user_id', $request->user_id)
        ...
        if ($user) {
            ...
            return response()->json(['status' => 'failed', 'message' => 'User not found in the group.']);
        }
    }
    ...
    public function LeaveGroup(Request $request)
    {
        ...
        if (!empty($user)) {
            ...
            return response()->json(['status' => 'failed','notPermitted' => false, 'message' => 'User not found in group.']);
        }
    }
    ...
    public function readOnly(Request $request)
    {
        ...
        if ($permitted){
            ...
            return response()->json([
                'status' => 'failed',
                'notPermitted' => true,
                'message' => 'Only admin can set to read only.'
            ]);
        }
