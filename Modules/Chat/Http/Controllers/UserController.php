<?php
	...
	class UserController extends Controller
	{
		...
		{
			try {
				return User::when(is_string($request->search), function ($q) use ($request) {
					$q->where('full_name', 'like', '%' . $request->search . '%')->orWhere('email', 'like', '%' . $request->search . '%');
				})->get()
				->map(function ($value) {
					return [
					'name' => $value->full_name,
					'route' => "/chat/user/search?keywords=".$value->full_name,
					// student_view
					];
				});
				} catch (\Exception $e) {
