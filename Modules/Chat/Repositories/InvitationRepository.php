...
class InvitationRepository
{
    ...
    {
        $to = Conversation::where('from_id', auth()->id())->pluck('to_id')->toArray();
        $from = Conversation::where('to_id', auth()->id())->pluck('from_id')->toArray();
        ...
        foreach ($users as $user){
            ...
            $unreadChats = $user->userSpecificConversationCollectionUnread($user->id);
            if ($last){
                $user->custom_order = $last->id;
                if ($last->message_type == 0){
                    $user->last_message = $last->message;
                    $date = new \DateTime($last->created_at);
                    $user->last_message_date = $date->format("d-m-Y h:i A");
                    $user->unread_chats = $unreadChats;
                }else{
