<?php
	...
	class FeesController extends Controller
	{
		...
		public function feesInvoiceView($id, $state)
		{
			...
			$invoiceInfo = FmFeesInvoice::find($id);
			$student = SmStudent::where('id',$invoiceInfo->student_id)->with('parents')->first();
			$invoiceInfo->invoice_id = str_replace('-','',$invoiceInfo->invoice_id);
			
			$qrCodeOptions = new QrCodeOptions;
			$qrCodeOptions->format("png");
			$qrCodeOptions->size(150);
			...
			$qrCode = Zatca::sellerName('Metashops')
            ->vatRegistrationNumber("123456789123456")
            ->timestamp(date('Y-m-d H:i:s'))
            ->totalWithVat(($paid_amount) + ((15 % 100) * $amount))
            ->vatTotal((15 / 100) * $amount)
            ->toQrCode($qrCodeOptions);
			$qrCode = base64_encode($qrCode);
			if ($state == 'view') {
			echo app()->getLocale();
				if(app()->getLocale() == 'en') {
					return view('fees::feesInvoice.feesInvoiceView', compact('generalSetting', 'invoiceInfo', 'invoiceDetails', 'banks','qrCode','student'));
					} else {
					return view('fees::feesInvoice.feesInvoiceViewAR', compact('generalSetting', 'invoiceInfo', 'invoiceDetails', 'banks','qrCode','student'));
				}
				
				} else {
				if(app()->getLocale() == 'en') {
					return view('fees::feesInvoice.feesInvoicePrint', compact('generalSetting','invoiceInfo', 'invoiceDetails', 'banks','qrCode','student'));
					} else {
					return view('fees::feesInvoice.feesInvoicePrintAR', compact('generalSetting','invoiceInfo', 'invoiceDetails', 'banks','qrCode','student'));
				}
			}
		}
