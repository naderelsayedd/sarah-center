    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="row mt-25 align-items-center">
                <div class="col-lg-12">
					<label id="teacher_label">@lang('common.enter_lesson')</label>
					<input type="text" class="form-control" name="lesson" id="lessonTopic" value="">
                    <!--<select
            ...
            <div class="row mt-25 align-items-center">
				...
				id="select_topic_div">
				<label id="teacher_label">@lang('common.enter_topic')</label>
...
<script type="text/javascript">
    $(document).ready(() => {
        ...
        $('#addRowTopic').click(function () {
			var lesson_id = $('#lessonTopic').val();
			if (lesson_id == '') {
				setTimeout(function() {
					toastr.error('Enter Lesson First', "Error", {
						timeOut: 5000,
					});
				}, 500);
				return;
			}
            var dynamicRowHTML = '<div class="rowClass" style="width: 100%; margin: 15px;"><div class="row"><div class="col-lg-10"><input type="text" class="form-control" name="topic[]" id="topic"></div><div class="col-2"><button class="remove primary-btn icon-only fix-gr-bg" type="button"> <span class="ti-trash"></span> </button></div></div></div>';
            $('#topicRow').append(dynamicRowHTML);
            count++;
		});
		
        $('#topicRow').on('click', '.remove', function () {
            $(this).parent('div').parent('.rowClass').remove(); 
		});
	})
