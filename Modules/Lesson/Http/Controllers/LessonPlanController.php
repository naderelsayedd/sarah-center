<?php
	...
	class LessonPlanController extends Controller
	{
		...
		public function index(Request $request)
		{
			...
			if(Auth::user()->role_id == "4"){
				...
				try {
					...
					$teachersData = SmStaff::select('id', 'full_name')
		...
		public function addNewLessonPlan(Request $request)
		{
			...
			try {
				$sections = SmAssignSubject::where('class_id', $request->class_id)
				...
				foreach ($sections as $section) {
                    ...
                    $smLesson->lesson_title = $request->lesson;
                    $smLesson->class_id = $request->class_id;
                    $smLesson->subject_id = $request->subject_id;
				...
				$lessonPlanner->lesson_detail_id = $lessonId;
				if ($request->customize != "customize") {
					$lessonPlanner->topic_id = $smTopic_id;
					$lessonPlanner->sub_topic = $request->sub_topic;
					$lessonPlanner->topic_detail_id = $smTopic_id;
				}
                ...
                $smTopic->class_id = $request->class_id;
                $smTopic->section_id = $request->section_id;
                $smTopic->subject_id = $request->subject_id;
                $smTopic->lesson_id = $lessonId;
                ...
                for ($i = 0; $i < $length; $i++) {
                    ...
                    $topicDetail->lesson_id = $lessonId;
					...
					$LessonPlanTopic->topic_id = $topicDetailId;
					$LessonPlanTopic->sub_topic_title = '';
		...
		public function manageLessonPlanner()
		{
			...
			try {
				...
				if(Auth::user()->role_id == "4"){
					...
					$getTeacherID = SmStaff::where('role_id', 4)->where('user_id', Auth::user()->id)->first();
					
					$assigned_class_teacher_check = SmClassTeacher::where('active_status', 1)
					->where('teacher_id', $getTeacherID->id)
					->first();
					
					$SmAssignClassTeacher = SmAssignClassTeacher::where('active_status', 1)
					->where('id', $assigned_class_teacher_check->assign_class_teacher_id)
					->first();
					
					$classes = SmClass::where('id',$SmAssignClassTeacher->class_id)->get();
