<?php
	...
	use App\Models\StudentAdvanceEvaluation;
	class SmParentPanelController extends Controller
	{
		...
		public function parentDashboard()
		{
			try {
				...
				if (!Auth::user()->is_approved) {
					Toastr::error('Your account is not approved by administrator. Please complete your child required details then admin will view and approved', 'Failed');
					$parent_info = Auth::user()->parent;
					$student_detail = SmStudent::where('parent_id', $parent_info->id)->first();
					if ($student_detail) {
						return redirect()->route('update-my-children', [$student_detail->id]);
					}					
				}
				...
				$interview = 1;
				$parent_data = SmParent::where('user_id',Auth::user()->id)->first();
				if ($parent_data->interview_status == '') {
					$interview = 0;
				}

				return view('backEnd.parentPanel.parent_dashboard', compact('holidays', 'calendar_events', 'events', 'totalNotices', 'my_childrens', 'sm_weekends', 'currency', 'complaints','interview'), $data);
				} catch (\Exception $e) {
		...
		public function studentUpdate(SmStudentAdmissionRequest $request)
		{
			try {
				...
				if (!Auth::user()->is_approved) {
					Toastr::error('You have update your child details successfully. Administrator will review your child profile and approve by administrator then you can access your account', 'Success');
					$parent_info = Auth::user()->parent;
					$student_detail = SmStudent::where('parent_id', $parent_info->id)->first();
					...
					$notification2->message = 'Parent '. $request->fathers_name. ' has been update his child' .$request->first_name.' '.$request->last_name .' details for review';
					$notification2->url = 'student_approval_pending_list';
		...
		public function parentConfirmation(Request $request) {
			try {
				$user = User::find(Auth::user()->id);
				$user->is_confirmation = $request->is_confirmation;
				$user->is_photo_confirmation = $request->is_photo_confirmation;
				$user->snapchat_url = '';
				if($request->is_photo_confirmation == 2){
				...
				$user = User::find($student_detail->user_id);
				$user->is_confirmation = $request->is_confirmation;
				$user->is_photo_confirmation = $request->is_photo_confirmation;
				$user->snapchat_url = '';
				if($request->is_photo_confirmation == 2){
				...
				Toastr::success('Parent Confirmtion Done', 'Success');
				return redirect()->back();
			} catch (\Exception $e) {
				Toastr::error($e->getMessage(), 'Failed');
				return redirect()->back();
		...
		public function myChildren($id)
		{
			try {
				...
				if ($student_detail) {
					...
					$studentEvaluation = StudentAdvanceEvaluation::where('status', 1)->where('student_id', $id)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)
                ->get();
		...
		public function leaveApply(Request $request)
		{
			try {
				...
				$leave_types = SmLeaveDefine::where('role_id', 2)->whereIn('user_id', $std_id->pluck('user_id'))->where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
		...
		public function teacherList($id)
		{
			try {
				...
				$teacherEvaluation = TeacherEvaluation::where('status','1')->get();
				$teacher_id = array();
				if (!$teacherEvaluation->isEmpty()) {
					foreach ($records as $record) {
						foreach ($record->StudentTeacher as $value) {
							if($teacherEvaluation->count() > 0){
								foreach ($teacherEvaluation as $teacherEval) {
									if($value->teacher_id == $teacherEval->teacher_id){
										$teacher_id[] = $teacherEval->teacher_id;
									}
								}
							}
						}
					}
					$teacherIds = count($teacher_id)?array_unique($teacher_id):array();
					}else{
					if($records->count() > 0){
						foreach ($records as $record) {
						}
				...
				$arr = array();
				$arrFinal = array();
				$getArray = $record->StudentTeacher;
				if($record->StudentTeacher->count() > 0){				
					foreach($getArray as $key=>$val){
						if(in_array($val->teacher_id, $arr)){
							$arr[$val->teacher_id][] = $val->subject->subject_name;
				...
				if(count($arrFinal) > 0){
					foreach($getArray as $keys=>$values){
						foreach($arrFinal as $key=>$val){
							if(!in_array($values->teacher_id, $setArr)){
								$setArr[] = $values->teacher_id;
								$setFinalArr[$keys] = $values;							
								$setFinalArr[$keys]->subject->subject_name = $arrFinal[$values->teacher_id];
								
								//$setFinalArr['subject']['subject_name'] = $values ;
							}
				...
				return view('backEnd.parentPanel.teacher_list', compact('records', 'student_detail', 'teacherEvaluationSetting','getQuestion','teacherIds','setFinalArr'));
				} catch (\Exception $e) {
		...
		public function interviewStore(Request $request)
	    {
	        ...
	        if ($interview->save()) {
	        	SmParent::where('user_id',Auth::user()->id)->update(['interview_status'=>1]);
		        Toastr::success('Operation successful', 'Success');
