...
class DatatableQueryController extends Controller
{
           ...
           $all_students =  SmStudent::with('studentRecords','studentRecords.class','studentRecords.section')->whereIn('id',$student_records)
            ...
            return Datatables::of($all_students)
                ...
                ->addColumn('action', function ($row) {
					...
					$btn = (userPermission('student.assign-class') === true ?'<a class="primary-btn small fix-gr-bg" target="_blank" href="' . route('student.assign-class', [$row->id]) . '" title="'.$langName.'"><i class="ti-thumb-up" style="font-size: 20px;"></i></a>' :'')
                        .((userPermission('student.assign-class') === true && moduleStatusCheck('University')) ? '<a class="primary-btn small fix-gr-bg" href="' . route('student_view', [$row->id,'assign_subject']) . '" title="'.app('translator')->get('university::un.assign_subject').'"><i class="ti-eye" style="font-size: 20px;"></i></a>' : '') .
						
						'<a href="' . route('student_view', [$row->id]) . '" class="primary-btn small fix-gr-bg" title="'.app('translator')->get('common.view').'"    ><i class="ti-view-grid" style="font-size: 20px;"></i></a>'.
						(userPermission('student_edit') === true ?'<a class="primary-btn small fix-gr-bg" target="_blank" href="' .  route('student_edit', [$row->id])  . '" title="'.app('translator')->get('common.edit').'"><i class="ti-pencil" style="font-size: 20px;"></i></a>' :'').
                         (userPermission('disabled_student') === true ? (Config::get('app.app_sync') ? '<span  data-toggle="tooltip" title="Disabled For Demo "><a  class="primary-btn small fix-gr-bg" href="#"  >' . app('translator')->get('common.disable') . '</a></span>' :
                            '<a onclick="deleteId(' . $row->id . ');" class="primary-btn small fix-gr-bg" href="#" data-toggle="modal" data-target="#deleteStudentModal" data-id="' . $row->id . '"  title="'.app('translator')->get('common.disable') .'" ><i class="ti-trash" style="font-size: 20px;"></i></a>') : '');
					/*$btn = (userPermission('student.assign-class') === true ?'<a class="primary-btn small fix-gr-bg" target="_blank" href="' . route('student.assign-class', [$row->id]) . '" title="'.$langName.'"><i class="ti-star" style="font-size: 20px;"></i></a>': '')
                        .((userPermission('student.assign-class') === true && moduleStatusCheck('University')) ? '<a class="primary-btn small fix-gr-bg" href="' .  route('student_view', [$row->id,'assign_subject']) . '" title="'.app('translator')->get('university::un.assign_subject').'"><i class="ti-eye" style="font-size: 20px;"></i></a>' : '')						
						 .'<a class="primary-btn small fix-gr-bg" href="' . route('student_view', [$row->id]) . '"><i class="ti-eye" style="font-size: 20px;"></i></a>' : ''
						.(userPermission('student_edit') === true ?'<a class="primary-btn small fix-gr-bg" target="_blank" href="' .  route('student_edit', [$row->id]) . '" title="'.app('translator')->get('common.edit').'"><i class="ti-pencil" style="font-size: 20px;"></i></a>': '')
                         (userPermission('disabled_student') === true ? (Config::get('app.app_sync') ? '' :
                            '<a title="Disabled For Demo " class="primary-btn small fix-gr-bg" href="#" data-toggle="modal" data-target="#deleteStudentModal" data-target="#deleteStudentModal" data-id="' . $row->id . '"  ><i class="ti-trash" style="font-size: 20px;"></i></a>') : '');*/
    ...
    {  
        try {
            ...
            return Datatables::of($staffs)
                ->addIndexColumn()
				->addColumn('full_name', function ($row) {
                    $btn = '<a class="" target="_blank" href="' . route('viewStaff', [$row->id]) . '">' . $row->full_name . '</a>';
                    return $btn;
                })
                ...
                ->addColumn('action', function ($row) {
                    $btn = '<a class="primary-btn small fix-gr-bg" target="_blank" href="' . route('viewStaff', [$row->id]) . '"><i class="ti-view-grid" style="font-size: 20px;"></i></a>' .
                        (userPermission('editStaff') === true ? '<a class="primary-btn small fix-gr-bg" href="' . route('editStaff', [$row->id]) . '"><i class="ti-pencil" style="font-size: 20px;"></i></a>' : '') .

                        (userPermission('deleteStaff') === true ? ($row->user_id == Auth::id() ? '' :
                            '<a onclick="deleteStaff(' . $row->id . ');" class="primary-btn small fix-gr-bg" href="#" data-toggle="modal" data-target="#deleteStudentModal" data-id="' . $row->id . '"  ><i class="ti-trash" style="font-size: 20px;"></i></a>') : '');

                    return $btn;
                })
                ->rawColumns(['action', 'switch','full_name'])
                ->make(true);
    ...
    public function ajaxExpenseList(Request $request){
        ...
        public function itemReceiveListAjax(Request $request){
		 ...
		 public function evaluationQuestionnairListAjax(Request $request){
            if($request->ajax()){
                $items = SmTeacherQuestionnair::where('school_id',Auth::user()->school_id);				
                return Datatables::of($items)
                ->addIndexColumn()
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<div class="dropdown CRM_dropdown">
                                    <button type="button"  class="btn dropdown-toggle" data-toggle="dropdown">' . app('translator')->get('common.select') . '</button>
                                    <div class="dropdown-menu dropdown-menu-right">'
                        .(userPermission('item-list-edit') === true ? '<a class="dropdown-item edit-item"  attr-id="'.$row->id.'">' . __('common.edit') . '</a>' :'')
        
                        .((userPermission('delete-item-view') === true) ?
                        '<a  class="dropdown-item delete-questionnair" set-href="'.route('delete-questionnair',@$row->id).'">' .  app('translator')->get('common.delete') . '</a>' :'') . 
