           ...
           $all_students =  SmStudent::with('studentRecords','studentRecords.class','studentRecords.section')->whereIn('id',$student_records)
            ...
            return Datatables::of($all_students)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = (userPermission('student.assign-class') === true ? '<a class="primary-btn small fix-gr-bg" target="_blank" href="' . route('student.assign-class', [$row->id]) . '"><i class="ti-view-grid" style="font-size: 20px;"></i></a>' :'')

                        .((userPermission('student.assign-class') === true && moduleStatusCheck('University')) ?
                        '<a class="primary-btn small fix-gr-bg" target="_blank" href="' . route('student_view', [$row->id,'assign_subject']) . '"><i class="ti-view-grid" style="font-size: 20px;"></i></a>' :'')

                        .'<a class="primary-btn small fix-gr-bg" target="_blank" href="' . route('student_view', [$row->id]) . '"><i class="ti-view-grid" style="font-size: 20px;"></i></a>' .

                        (userPermission('student_edit') === true ? '<a class="primary-btn small fix-gr-bg" href="' . route('student_edit', [$row->id]) . '"><i class="ti-pencil" style="font-size: 20px;"></i></a>' : '') .
                        ...
                        (userPermission('disabled_student') === true ? (Config::get('app.app_sync') ? '<span  data-toggle="tooltip" title="Disabled For Demo "><a  class="primary-btn small fix-gr-bg" href="#"  >' . app('translator')->get('common.disable') . '</a></span>' :
                            '<a onclick="deleteId(' . $row->id . ');" class="primary-btn small fix-gr-bg" href="#" data-toggle="modal" data-target="#deleteStudentModal" data-id="' . $row->id . '"><i class="ti-trash" style="font-size: 20px;"></i></a>') : '');
