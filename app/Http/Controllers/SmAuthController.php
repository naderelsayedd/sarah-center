<?php
	...
	class SmAuthController extends Controller
	{
		...
		public function accountActivationConfirtmation($email, $code)
		{
			try {
				$user = User::where('email', $email)->where('random_code', $code)->first();
				if ($user != "") {
					...
					$result = $user->save();
					if ($result) {
						Toastr::success('Your account has been activated successfully', 'Success');
						return redirect('login')->with('message-success', 'Your account has been activated successfully');
						} else {
						Toastr::error('Operation Failed', 'Failed');
