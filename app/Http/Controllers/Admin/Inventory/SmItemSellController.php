...
class SmItemSellController extends Controller
{
    ...
    public function itemSell(Request $request)
    {
        try {
            ...
            return view('backEnd.inventory.itemSell', compact('suppliers', 'items', 'paymentMethhods', 'roles','classes', 'sell_heads','account_id','students'));
        } catch (\Exception $e) {
            ...
            if ($results) {
                ...
                for ($i = 0; $i < $item_ids; $i++) {
                    if (!empty($request->item_id[$i])) {
						...
						$itemSellChild->descriptionItem = $request->descriptionItem[$i];
						$itemSellChild->discount = $request->discount[$i];
						$itemSellChild->vat = $request->vat[$i];
    ...
    public function viewItemSell($id)
    {
        try {
            ...
            ->totalWithVat(number_format( (float) $viewData->total_paid, 2, '.', ''))
            ->vatTotal((15 / 100) * number_format( (float) $viewData->total_paid, 2, '.', ''))
            ->toQrCode($qrCodeOptions);
			$qrCode = base64_encode($qrCode);
			
            return view('backEnd.inventory.viewItemSell', compact('viewData', 'editDataChildren', 'general_setting','qrCode'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function editItemSell(Request $request, $id)
    {
        try {
            ...
            if ($editData->role_id == 2 && $editData->student_staff_id) {
