	...
	class SmStudentAdmissionController extends Controller
	{
		...
		public function store(SmStudentAdmissionRequest $request)
		{
			...
			if ($request->filled('phone_number') || $request->filled('email_address')) {
				...
				if ($request->guardians_email && $request->email_address && $request->email_address == $request->guardians_email) {
					return redirect()->back()->with('message-danger', "Parent and Child Email can not be same, Please try again");
				}
				...
				if(!empty($request->email_address)) {
					$emailChildEmailCheck = User::select('*')->where('email', $request->email_address)->first();
					if (!empty($emailChildEmailCheck)) {
						return redirect()->back()->with('message-danger', "Child Email Already Exist, Please try an other email id again");
					}
				}
				
				if(!empty($request->phone_number)) {
					$emailChildPhoneCheck = User::select('*')->where('phone_number', $request->phone_number)->first();
					if (!empty($emailChildPhoneCheck)) {
						return redirect()->back()->with('message-danger', "Child Phone Number Already Exist, Please try an other phone id again");
					}
				}
			...
			try {
				...
				$user_stu->full_name = $request->first_name;
				$user_stu->username = $request->phone_number ?: ($request->email_address ?: $request->admission_number);
				$user_stu->email = !empty($request->email_address) ? $request->email_address : $childRandamNumber.$request->guardians_email;
				...
				if ($request->parent_id == "") {
					...
					if ($parentInfo && !$request->staff_parent) {
						...
						$parent->guardians_address_lat = $request->guardians_address_lat;
						$parent->guardians_address_long = $request->guardians_address_long;
				...
				$student->current_address_lat = $request->current_address_lat;
				$student->permanent_address_lat = $request->permanent_address_lat;
				$student->current_address_long = $request->current_address_long;
				$student->permanent_address_long = $request->permanent_address_long;
		...
		public function view(Request $request, $id, $type = null)
		{
			try {
				...
				if (!empty($student_detail->vechile_id)) {
					$vehicle_info = SmVehicle::where('id', '=', $student_detail->vechile_id)->first();
					$driver_info = SmStaff::where('id', '=', $vehicle_info->driver_id)->first();
					} else {
					$vehicle_info = '';
					$driver_info = '';
				}
		...
		public function studentTeacherEvaluation(Request $request) {
			try {
				foreach($request->evaluation as $key => $value) {
					$evaluation = new StudentAdvanceEvaluation();
					$evaluation->role_id = $request->role_id;
					$evaluation->role_id = $request->role_id;
					$evaluation->teacher_id = $request->teacher_id;
					$evaluation->student_id = $request->student_id;
					$evaluation->parent_id = $request->parent_id;
					$evaluation->is_eligible = $request->is_eligible;
					$evaluation->evaluation = $request->evaluation[$key];
					$evaluation->versed = !empty($request->versed[$key]) ? 1 : 0;
					$evaluation->extend = !empty($request->extend[$key]) ? 1 : 0;
					$evaluation->cannot = !empty($request->cannot[$key]) ? 1 : 0;
					$evaluation->status = 1;
					$evaluation->academic_id = getAcademicId();
					$evaluation->school_id = Auth::user()->school_id;
					$evaluation->save();
				}
				Toastr::success('Operation successful', 'Success');
				return redirect()->back()->with(['advance_evaluation' => 'active']);
				} catch (\Throwable $th) {
				throw $th;
				DB::rollback();
				Toastr::error('Operation Failed', 'Failed');
		...
		public function studentPendingList(Request $request)
		{
			try {
				$pt = trans('student.student_approval_pending_list');
				$usersId = [];
				...
				$parents = SmParent::whereIn('user_id', $usersId)->get();
				$usersId = [];
				foreach($parents as $parent) {
					$usersId[] = $parent->id;
				}
				$students = SmStudent::whereIn('parent_id', $usersId)->where('school_id', Auth::user()->school_id)->where('academic_id', getAcademicId())->get();
				$classes = SmClass::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
				return view('backEnd.studentInformation.unapprove_student', compact('students', 'classes', 'pt'));
				} catch (\Exception $e) {
				Toastr::error('Operation Failed', 'Failed');
				return redirect()->back();
			}
