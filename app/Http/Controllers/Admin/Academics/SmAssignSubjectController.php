	...
	class SmAssignSubjectController extends Controller
	{
		...
		public function search(Request $request)
		{
			...
			try {
				...
				$assign_class_teachers = SmAssignClassTeacher::with('class', 'section', 'classTeachers')->where('class_id',$request->class)->where('academic_id', getAcademicId())->status()->orderBy('class_id', 'ASC')->orderBy('section_id', 'ASC')->first();
				$teacherIds = [];
				if(!empty($assign_class_teachers)) {
					$classTeachers = $assign_class_teachers->classTeachers;
					if($classTeachers != "") {
				...
				if(!empty($teacherIds)) {
					...
					->where(function ($q) {
						$q->where('role_id', 4)->orWhere('previous_role_id', 4);
					})->where('school_id', Auth::user()->school_id)->whereIn('id',$teacherIds)->get();
				} 
		...
		public function assignSubjectAjax(Request $request)
		{
			...
			try {
				...
				$assign_class_teachers = SmAssignClassTeacher::with('class', 'section', 'classTeachers')->where('class_id',$request->class_id)->where('academic_id', getAcademicId())->status()->orderBy('class_id', 'ASC')->orderBy('section_id', 'ASC')->first();
				$teacherIds = [];
				if(!empty($assign_class_teachers)) {
					$classTeachers = $assign_class_teachers->classTeachers;
					if($classTeachers != "") {
