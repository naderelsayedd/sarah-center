	...
	class SmClassRoutineNewController extends Controller
	{
		...
		public function classRoutineSearch(Request $request)
		{
			...
			try {
				...
				$assign_class_teachers = SmAssignClassTeacher::with('class', 'section', 'classTeachers')->where('class_id',$class_id)->where('academic_id', getAcademicId())->status()->orderBy('class_id', 'ASC')->orderBy('section_id', 'ASC')->first();
				...
				$teachers = SmStaff::where('role_id', 4)->whereIn('id',$teacherIds)->where('school_id', Auth::user()->school_id)->get(['id', 'full_name', 'user_id']);
			...
			{	
				if(Auth::user()->role_id == "4"){
					try {
			            ...
			            $teachers = SmStaff::where('role_id', 4)->select('id', 'full_name')
			            	->where('active_status', 1)
			            	->where('user_id', Auth::user()->id)
			            	->where('school_id', Auth::user()->school_id)->get();
					...
					try {
			            $teachers = SmStaff::select('id', 'full_name')
			            ->where('active_status', 1)
			            ->where(function($q)  {
						$q->where('role_id', 4)->orWhere('previous_role_id', 4);
			            })->where('school_id', Auth::user()->school_id)->get();
