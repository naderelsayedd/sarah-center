        ...
        try {
            $assigned_class_teacher = SmAssignClassTeacher::where('active_status', 1)
			 ...
			 if (empty($assigned_class_teacher)) {
                $assigned_class_teacher = new SmAssignClassTeacher();
                $assigned_class_teacher->class_id = $request->class;
                $assigned_class_teacher->section_id = $request->section;
                $assigned_class_teacher->school_id = auth()->user()->school_id;
                $assigned_class_teacher->academic_id = getAcademicId();
                $assigned_class_teacher->save();
			 }
			 $assigned_class_teacher_check = SmClassTeacher::where('active_status', 1)
                ->where('assign_class_teacher_id', $assigned_class_teacher->id)
                ->where('teacher_id', $request->teacher)
                ->where('academic_id', getAcademicId())
                ->where('school_id', auth()->user()->school_id)
                ->first();
			
            if (empty($assigned_class_teacher_check)) {
                $class_teacher = new SmClassTeacher();
                $class_teacher->assign_class_teacher_id = $assigned_class_teacher->id;
                ...
                $class_teacher->save();
                event(new ClassTeacherGetAllStudent($assigned_class_teacher, $class_teacher));
                DB::commit();
