    ...
    public function percentMarkSheetReport(PercentMarkSheetReportRequest $request)
    {
        try {
            ...
            if ($exam_rule) {
                ...
                $mark_sheet->where('student_id', $request->student_id);
                if (moduleStatusCheck('University')) {
                ...
                return view('backEnd.examination.report.marksheetReport', compact('exams', 'classes','SmStudent','mark_sheet', 'examInfo', 'subjectInfo', 'classInfo', 'sectionInfo', 'pass_mark', 'exam_rule', 'data'));
            } else {
                $mark_sheet = SmResultStore::query();
                $mark_sheet->where('exam_type_id', $request->exam_type);
                $mark_sheet->where('student_id', $request->student_id);
