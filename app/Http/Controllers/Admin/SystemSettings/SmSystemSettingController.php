	...
	class SmSystemSettingController extends Controller
	{
		...
		public function updateGeneralSettingsData(SmGeneralSettingsRequest $request)
		{
			...
			try {
				...
				$generalSettData->vat = $request->vat;
				$generalSettData->lat = $request->lat;
				$generalSettData->long = $request->long;
				$generalSettData->range_in_miles = $request->range_in_miles;
				$generalSettData->phone = $request->phone;
				$generalSettData->email = $request->email;
				$generalSettData->vat_on_fees = $request->vat_on_fees;
		...
		public function setUserTheme(Request $request) {
			try {
				...
				$user->theme_id = $request->theme_name;
				$user->save();
		...
		public function themeStyleActive(Request $request)
		{
			...
			try {
				...
				if ($request->id) {
                    ...
                    $selected = Theme::where('id', $request->id)->first();
		...
		public function setUserAttendance(Request $request) {
			try {
				$editData = generalSetting();
				if($editData->lat && $editData->long) {
					$theta = $request->checkin_long - $editData->long;
					$dist = sin(deg2rad($request->checkin_lat)) * sin(deg2rad($editData->lat)) +  cos(deg2rad($request->checkin_lat)) * cos(deg2rad($editData->lat)) * cos(deg2rad($theta));
					$dist = acos($dist);
					$dist = rad2deg($dist);
					$miles = $dist * 60 * 1.1515;
					$miles = (int)($miles * 1.609344);
					
					if($miles <= $editData->range_in_miles) {
					
					} else {
						return response()->json(['error' => 'Sorry! You are too much far '.$miles.' Kilometer from your office address. Attendance allow only '.$editData->range_in_miles.' Kilometer in Range', 'attendance' => '']);
					}

				}
				
				$attendance = new SmAttendance();
				$attendance->user_id = Auth::user()->id;
				$attendance->in_time = date('Y-m-d H:i:s');
				$attendance->status = 'incomplete';
				$attendance->checkin_lat = $request->checkin_lat;
				$attendance->checkin_long = $request->checkin_long;
				$attendance->checked_at = date('Y-m-d H:i:s');
				$attendance->school_id = Auth::user()->school_id;
				$attendance->academic_id = getAcademicId();
				$attendance->save();
				/*Minus the subscription plan count on clockin user starts from here*/
				$today = now()->format('Y-m-d');
				User::where('id',Auth::user()->id)
				    ->where('role_id',3)
				    ->where('subscription_plan_days_remaining', '>', 0)
				    ->whereDate('subscription_plan_last_updated', '<>', $today)
				    ->decrement('subscription_plan_days_remaining', 1);
				/*Minus the subscription plan count on clockin user ends here*/
				return response()->json(['attendance' => $attendance,'time' => date('h:i A', strtotime($attendance->in_time))]);
				} catch (\Exception $e) {
				return response()->json(['error' => $e->getMessage(), 'attendance' => '']);
				}
			}
			
			public function setUserAttendanceCheckout(Request $request) {
				try {
					...
					$attendance = SmAttendance::find($request->id);
					$attendance->user_id = Auth::user()->id;
					$attendance->out_time = date('Y-m-d H:i:s');
					$attendance->status = 'pending';
					$attendance->note = $request->note;
					$attendance->checkout_lat = $request->checkin_lat;
					$attendance->checkout_long = $request->checkin_long;
					$attendance->save();
					return response()->json(['attendance' => $attendance,'time' => date('h:i A', strtotime($attendance->in_time))]);
					} catch (\Exception $e) {
					return response()->json(['error' => $e->getMessage(), 'attendance' => '']);
