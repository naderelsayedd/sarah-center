					...
					if (empty($request->selectTab) or $request->selectTab == 'G') {
						if (empty($request->role)) {
							...
							if ($request->send_through == 'E') {
								...
								return redirect()->back();
								} else if ($request->send_through == 'W') {
								$receiverDetails = '';
								foreach ($request->role as $role_id) {
									if ($role_id == 2) {
										$receiverDetails = SmStudent::select('email', 'full_name', 'mobile')
									...
									foreach ($receiverDetails as $receiverDetail) {
										if($receiverDetail->mobile) {
											@sendWhatsappMessage($request->description, 'text', $receiverDetail->mobile);
										}
									}
								}
								Toastr::success('Operation successful', 'Success');
						...
						if (empty($request->message_to_individual)) {
							...
							if ($request->send_through == 'E') {
								...
								foreach ($message_to_individual as $key => $value) {
									...
									if($receiver_mobile) {
									print_r($receiver_mobile);
										@sendWhatsappMessage($request->description, 'text', $receiver_mobile);
								...
								foreach ($message_to_individual as $key => $value) {
									...
									$getParentUserId = SmParent::where('guardians_email', $to_email)->first();
									$userData = User::find($getParentUserId->user_id);
									$device_token = $userData->device_token;
									
									$body['title'] = $request->email_sms_title;
									$body['type'] = 'parent';
									$body['message'] = $request->description;
									$title = $request->email_sms_title;;
									$response = PushNotification('Android',true,$device_token,$title,$body);
									...
									foreach ($selectedSections as $key => $value) {
										...
										if ($message == 2) {
											...
											foreach ($students as $student) {
												...
												$to_mobile[] = $student->mobile;
												if($student->mobile) {
													@sendWhatsappMessage($request->description, 'text', $student->mobile);
												}
											}
										}
										if ($message == 3) {
											...
											foreach ($parents as $parent) {
												...
												$to_email[] = $parent->parents->guardians_email;
												$to_mobile[] = $parent->parents->guardians_mobile;
												if($parent->parents->guardians_mobile) {
													@sendWhatsappMessage($request->description, 'text', $parent->parents->guardians_mobile);
												}
											}
										}
									}
