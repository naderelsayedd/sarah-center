<?php
	...
	class SmStudentAdmissionController extends Controller
	{
		...
		public function enableStudent(Request $request)
		{
			try {
				...
				if ($parent) {
					...
					if(!$parent_user->is_approved) {
						$parent_user->is_approved = 1;
						$parent_user->save();
						$data['user_email'] = $parent->guardians_email;
						$data['id'] = $student_detail->parent_id;
						$data['role_id'] = 3;
						$data['admission_number'] = "";
						@send_mail($parent->guardians_email, $parent->fathers_name, "activate_account_confirmation", $data);
						$notification2 = new SmNotification;
						$notification2->user_id = $parent->user_id;
						$notification2->role_id = 3; 
						$notification2->message = 'Your Child has been Approved Now';
						$notification2->url = 'my-children/'.$student_detail->id;
						...
						$notification2->message = 'Student '.$student_detail->first_name.' '.$student_detail->last_name.' has been approve now';
						$notification2->url = 'student-view/'.$student_detail->id;;
						$notification2->date = date('Y-m-d');
						$notification2->school_id = app('school')->id;
						$notification2->academic_id  = getAcademicId();
						$notification2->save();
						...
						$postData['class_id'] = $student_detail->studentRecord->class_id;
						$postData['record_id'] = $student_detail->studentRecord->id;
						$postData['student_id'] = $student_detail->id;
						auto_generate_invoice($postData, $parent, $student_detail);
						
						/*$postDataEmail['user_email'] = $parent->guardians_email;
						$postDataEmail['id'] = $student_detail->parent_id;
						$postDataEmail['role_id'] = 3;
						$postDataEmail['admission_number'] = "";
						$postDataEmail['class_id'] = $student_detail->studentRecord->class_id;
						$postDataEmail['record_id'] = $student_detail->studentRecord->id;
						$postDataEmail['student_id'] = $student_detail->id;
						$postDataEmail['due_amount'] = 2500;
						$postDataEmail['fees_name'] = 'Student Course Registration Fees';
						$postDataEmail['date'] = date('Y-m-d');
						@send_mail($parent->guardians_email, $parent->fathers_name, "due_fees_payment", $postDataEmail);*/
					}
				}
				...
				Toastr::success('Student Account has been approved now', 'Success');
				return redirect()->back();
				} catch (\Exception $e) {
				DB::rollback();
				Toastr::error('Operation Failed', 'Failed');
		...
		public function unapproveStudent(Request $request)
		{
			try {
				if (isSubscriptionEnabled()) {
					$active_student = SmStudent::where('school_id', Auth::user()->school_id)->where('active_status', 1)->count();
				...
				if ($parent) {
					...
					$parent_user = User::find($parent->user_id);
					if(!$parent_user->is_approved) {
						$parent_user->not_approved_reason = $request->not_approved_reason;
						$parent_user->save();
						$data['user_email'] = $parent->guardians_email;
						$data['id'] = $student_detail->parent_id;
						$data['role_id'] = 3;
						$data['admission_number'] = "";
						$data['not_approved_reason'] = $request->not_approved_reason;
						@send_mail($parent->guardians_email, $parent->fathers_name, "activate_account_not_approved", $data);
						$notification2 = new SmNotification;
						$notification2->user_id = $parent->user_id;
						$notification2->role_id = 3; 
						$notification2->message = 'Your Child has not been Approved. Please check your email update child details';
						$notification2->url = 'update-my-children/'.$student_detail->id;
						$notification2->date = date('Y-m-d');
						$notification2->school_id = app('school')->id;
						$notification2->academic_id  = getAcademicId();
						$notification2->save();
						...
						$notification2->user_id = 1;
						$notification2->role_id = 1; 
						$notification2->message = 'Student '.$student_detail->first_name.' '.$student_detail->last_name.' not been Approved. Please check your email update child details';
						$notification2->url = 'student-view/'.$student_detail->id;;
						$notification2->date = date('Y-m-d');
						$notification2->school_id = app('school')->id;
						$notification2->academic_id  = getAcademicId();
						...
						Toastr::error('Student Account has not been approved now', 'Failed');
						return redirect()->back();
					}
				}
