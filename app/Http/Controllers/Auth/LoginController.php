...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if ($school->id != 1 && $school->active_status != 1) {
			...
			return redirect()->back()->with('message-danger', 'Your Institution is not Active, Please contact with administrator.');
        }
        ...
        if ($logged_in) {
            ...
            if (!$school->active_status) {
				...
				return redirect()->back()->with('message-danger', 'Your Institution is not Approved, Please contact with administrator.');
            }
            ...
            if (!Auth::user()->access_status || !Auth::user()->active_status) {
				...
				return redirect()->back()->with('message-danger', 'You are not allowed, Please contact with administrator.');
            }
			
			if (!Auth::user()->email_verify) {
                $this->guard()->logout();
				Toastr::error('Your account is not varified. Please check your registered email ID and click to activate your account', 'Failed');
				return redirect()->back()->with('message-danger', 'Your account is not varified. Please check your registered email ID and click to activate your account');
            }
    ...
    public function loginFormTwo()
    {
		...
		$data['courses'] = SmCourse::where('school_id', app('school')->id)->get();
    ...
    public function loginCodeCanyon()
    {
        ...
        if (generalSetting() &&  generalSetting()->active_theme == 'edulia') {
            return view('frontEnd.theme.' . activeTheme() . '.login.login', compact('css','courses'))->with($data);
        } else {
