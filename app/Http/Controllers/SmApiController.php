<?php
	...
	use App\SmStaffDriverTraking;
	use App\SmStaffDriverTrakingRoute;
	use App\SmStaffDriverStudentTrakingRoute;
	...
	class SmApiController extends Controller
	{
		...
		public function getDriverStudentList(Request $request)
		{
			$student_list = DB::table('sm_students')->select('id','first_name','last_name','full_name','mobile','driver_id','current_address','student_photo')->where('active_status', 1)->where('driver_id', '=', $request->staff_id)->get();
			/*$student_list;
				foreach ($driver_list as $key => $value) {
				if($value->id) {
				$student_list[] = DB::table('sm_students')->where('active_status', 1)->where('id', '=', $value->id)->get();
				}
			}*/
			if (ApiBaseMethod::checkUrl($request->fullUrl())) {
				return ApiBaseMethod::sendResponse($student_list, null);
			}
		}
		...
		public function getDriverTrakingList(Request $request)
		{
			if($request->type == 'admin') {
				$driver_list = DB::table('sm_staff_driver_trakings')->select('id','student_ids','address','address_end','traking_start','traking_end','driver_id','start_photo','end_photo','notes')->where('driver_id', '=', $request->staff_id)->get();
		...
		public function getDriverTrakingRouteList(Request $request)
		{
			$data['traking'] = DB::table('sm_staff_driver_trakings')->select('id','student_ids','address','address_end','traking_start','traking_end','driver_id','start_photo','end_photo','notes')->where('id', '=', $request->track_id)->get();
			if($request->type == 'admin' || $request->type == 'driver') {
				if($request->student_id && $request->track_id) {
					...
					$request->student_ids = str_replace('"','',$request->student_ids);
					$data['student'] = SmStudent::select('id','first_name','last_name','full_name','mobile','driver_id','current_address','student_photo')->whereIn('id', explode(',',$request->student_ids))->get();
				}
			}
		...
		public function sendNotificationWhenParentReached(Request $request) {
			
			
			if($request->type == 'pick') {
				$custommessage = ' has reached to school for pick-up his childs';
			}
			if($request->type == 'drop') {
				$custommessage = ' has leaving from school with his childs ';
			}
			...
			if(empty($getParentData)) {
				return ApiBaseMethod::sendError('Parent Not Exist', null);
			}
			...
			if (ApiBaseMethod::checkUrl($request->fullUrl())) {
				return ApiBaseMethod::sendResponse('Parent '.$parents->full_name.$custommessage.' at '.date('Y-m-d H:i:s'), null);
			}
			
		}
		
		public function sendNotificationWhenDriverReached(Request $request) {
			
			$getDriverDetails = DB::table('sm_staffs')->select('users.id', 'users.full_name','users.email','users.device_token','users.device_id')->join('users', 'users.id', '=', 'sm_staffs.user_id')->where('sm_staffs.id', str_replace('"','',$request->driver_id))->first();
			$request->student_ids = str_replace('"','',$request->student_ids);
			if(empty($getDriverDetails)) {
				return ApiBaseMethod::sendError('Driver Not Exist', null);
			}
			foreach(explode(',',$request->student_ids) as $value) {
				$getParentData = DB::table('sm_students')->select('sm_parents.user_id as parent_user_id')->join('sm_parents', 'sm_parents.id', '=', 'sm_students.parent_id')->where('sm_students.id', trim($value))->first();
				$parents = User::where('id', '=', $getParentData->parent_user_id)->select('id', 'full_name','email','device_token','device_id')->get();
				if(!empty($parents)) {
					foreach($parents as $adminValue) {
						if($adminValue->device_id) {
							$body['title'] = 'Driver '.$getDriverDetails->full_name.' has reached to school for pick-up thier childrens';
							$body['type'] = 'parent';
							$body['message'] = 'Driver '.$getDriverDetails->full_name.' has reached to school for pick-up thier childrens '.date('Y-m-d H:i:s');
							$title = 'Driver '.$getDriverDetails->full_name.' has reached to school for pick-up thier childrens';
							$device_id = $adminValue->device_token;
							$response = PushNotification('Android',true,$device_id,$title,$body);
						}
						...
						$notification->academic_id = SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
						$notification->date = date('Y-m-d');
						$notification->message = 'Driver '.$getDriverDetails->full_name.' has reached to school for pick-up thier childrens';
						$notification->save();
			...
			if(!empty($admins)) {
				foreach($admins as $adminValue) {
					if($adminValue->device_id) {
						$body['title'] = 'Driver '.$getDriverDetails->full_name.' has reached to school for pick-up thier childrens';
						$body['type'] = 'admin';
						$body['message'] = 'Driver '.$getDriverDetails->full_name.' has reached to school for pick-up thier childrens at '.date('Y-m-d H:i:s');
						$title = 'Driver '.$getDriverDetails->full_name.' has reached to school for pick-up thier childrens';
						$device_id = $adminValue->device_token; //'';
						$response = PushNotification('Android',true,$device_id,$title,$body);
					}
					...
					$notification->academic_id = SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
					$notification->date = date('Y-m-d');
					$notification->message = 'Driver '.$getDriverDetails->full_name.' has reached to school for pick-up thier childrens at '.date('Y-m-d H:i:s');
					$notification->save();
			...
			if (ApiBaseMethod::checkUrl($request->fullUrl())) {
				return ApiBaseMethod::sendResponse('Driver '.$getDriverDetails->full_name.' has started travelling to droping child at '.date('Y-m-d H:i:s'), null);
			}
			
		}
		
		public function driverTrakingStart(Request $request)
		{
			$trakings = new SmStaffDriverTraking();
			$request->latitute =  str_replace('"','',$request->latitute);
			$request->longitute =  str_replace('"','',$request->longitute);
			$uploadFile = "";
			if ($request->file('photo') != "") {
				...
				if ($fileSizeKb >= $maxFileSize) {
					return response()->json(['error'=>'Max upload photo size ' . $maxFileSize . ' Mb is set in system',]);
				}
				$file = $request->file('photo');
				$uploadFile = 'start-' . md5($file->getClientOriginalName() . time()) . "." . $file->getClientOriginalExtension();
				$file->move('public/uploads/student/document/', $uploadFile);
				$uploadFile = 'public/uploads/student/document/' . $uploadFile;
			}
			$request->student_ids = str_replace('"','',$request->student_ids);
			$request->driver_id = str_replace('"','',$request->driver_id);
			$trakings->student_ids = $request->student_ids;
			$trakings->start_photo = $uploadFile;
			$trakings->address = $request->address;
			$trakings->traking_start = date('Y-m-d h:i:s');
			$trakings->driver_id = $request->driver_id;
			$trakings->notes = $request->notes;
			$trakings->school_id = Auth::user()->school_id;
			$trakings->academic_id = SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
			$trakings->save();
			$trakingId = $trakings->id;
			

			$data['student'] = SmStudent::select('id','first_name','last_name','full_name','mobile','driver_id','current_address','student_photo')->whereIn('id', explode(',',$request->student_ids))->get();
			$getDriverDetails = DB::table('sm_staffs')->select('users.id', 'users.full_name','users.email','users.device_token','users.device_id')
			->join('users', 'users.id', '=', 'sm_staffs.user_id')
			->where('sm_staffs.id', $request->driver_id)
			->first();

			foreach(explode(',',$request->student_ids) as $value) {
				
				if($trakings) {
					$trakingStudentRoutes = new SmStaffDriverStudentTrakingRoute();
					$trakingStudentRoutes->track_id = $trakingId;
					$trakingStudentRoutes->student_id = $value;
					$trakingStudentRoutes->student_start_photo = $uploadFile;
					$trakingStudentRoutes->student_end_photo = '';
					$trakingStudentRoutes->start_datetime = date('Y-m-d h:i:s');
					$trakingStudentRoutes->student_address = $request->address;
					$trakingStudentRoutes->student_latitute = $request->latitute;
					$trakingStudentRoutes->student_longitute = $request->longitute;
					$trakingStudentRoutes->school_id = Auth::user()->school_id;
					$trakingStudentRoutes->academic_id = SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
					$trakingStudentRoutes->save();
					
					$getParentData = DB::table('sm_students')->select('sm_parents.user_id as parent_user_id')
					
					->join('sm_parents', 'sm_parents.id', '=', 'sm_students.parent_id')
					->where('sm_students.id', trim($value))
					->first();
					...
					if(!empty($parents)) {
						...
						foreach($parents as $adminValue) {
							if($adminValue->device_id) {
								$body['title'] = 'Driver '.$getDriverDetails->full_name.' has started travelling to droping child';
								$body['type'] = 'parent';
								$body['message'] = 'Driver '.$getDriverDetails->full_name.' has started travelling to droping child at'.date('Y-m-d H:i:s');
								$title = 'Driver '.$getDriverDetails->full_name.' has started travelling to droping child';
								$device_id = $adminValue->device_token;
								$response = PushNotification('Android',true,$device_id,$title,$body);
							}
							...
							$notification->academic_id = SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
							$notification->date = date('Y-m-d');
							$notification->message = 'Driver '.$getDriverDetails->full_name.' has started travelling to droping child at'.date('Y-m-d H:i:s');
							$notification->save();
			...
			if($trakings) {
				$trakingRoutes = new SmStaffDriverTrakingRoute();
				$trakingRoutes->track_id = $trakingId;
				$trakingRoutes->address = $request->address;
				$trakingRoutes->latitute = $request->latitute;
				$trakingRoutes->longitute = $request->longitute;
				$trakingRoutes->school_id = Auth::user()->school_id;
				$trakingRoutes->academic_id = SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
				$trakingRoutes->save();
			}
		...
		public function driverTrakingStartRoute(Request $request)
		{
			
			$trakingRoutes = new SmStaffDriverTrakingRoute();
			$request->latitute =  str_replace('"','',$request->latitute);
			$request->longitute =  str_replace('"','',$request->longitute);
			$trakingRoutes->track_id = $request->track_id;
			$trakingRoutes->address = $request->address;
			$trakingRoutes->latitute = $request->latitute;
			$trakingRoutes->longitute = $request->longitute;
			$trakingRoutes->school_id = Auth::user()->school_id;
			...
			if($request->student_ids) {
				...
				foreach(explode(',',$request->student_ids) as $value) {
					$trakingStudentRoutes = new SmStaffDriverStudentTrakingRoute();
					$trakingStudentRoutes->track_id = $request->track_id;
					$trakingStudentRoutes->student_id = $value;
					$trakingStudentRoutes->student_start_photo = '';
					$trakingStudentRoutes->student_end_photo = '';
					$trakingStudentRoutes->start_datetime = date('Y-m-d h:i:s');
					$trakingStudentRoutes->student_address = $request->address;
					$trakingStudentRoutes->student_latitute = $request->latitute;
			...
			if (ApiBaseMethod::checkUrl($request->fullUrl())) {
				return ApiBaseMethod::sendResponse($trakingRoutes, null);
			}
		}
		
		public function driverTrakingEnd(Request $request)
		{
			
			$request->track_id = str_replace('"','',$request->track_id);
			$trakings = SmStaffDriverTraking::find($request->track_id);
			$request->latitute =  str_replace('"','',$request->latitute);
			$request->longitute =  str_replace('"','',$request->longitute);
			$uploadFile = '';
			if ($request->file('photo') != "") {
			...
			$trakings->end_photo = $uploadFile;
			$trakings->traking_end = date('Y-m-d h:i:s');
			$trakings->address_end = $request->address;
			$trakings->save();
			if($trakings) {
				$trakingRoutes = new SmStaffDriverTrakingRoute();
				$trakingRoutes->track_id = $request->track_id;
			...
			if($request->student_ids) {
				...
				foreach(explode(',',$request->student_ids) as $value) {
					...
					$trakingStudentRoutes->student_id = $value;
					$trakingStudentRoutes->student_start_photo = '';
					$trakingStudentRoutes->student_end_photo = $uploadFile;;
					$trakingStudentRoutes->end_datetime = date('Y-m-d h:i:s');
					$trakingStudentRoutes->student_address = $request->address;
					$trakingStudentRoutes->student_latitute = $request->latitute;
					$trakingStudentRoutes->student_longitute = $request->longitute;
					$trakingStudentRoutes->school_id = Auth::user()->school_id;
		...
		public function driverTrakingDropStudent(Request $request)
		{
			$request->student_id = str_replace('"','',$request->student_id);
			$request->track_id = str_replace('"','',$request->track_id);
			$request->latitute =  str_replace('"','',$request->latitute);
			$request->longitute =  str_replace('"','',$request->longitute);
			$trakings = SmStaffDriverTraking::find($request->track_id);
			...
			if ($request->file('photo') != "") {
				...
				$file = $request->file('photo');
				$uploadFile = 'end-' . md5($file->getClientOriginalName() . time()) . "." . $file->getClientOriginalExtension();
				$file->move('public/uploads/student/document/', $uploadFile);
				$uploadFile = 'public/uploads/student/document/' . $uploadFile;
			}
			...
			$trakingStudentRoutes = new SmStaffDriverStudentTrakingRoute();
			$trakingStudentRoutes->track_id = $request->track_id;
			$trakingStudentRoutes->student_id = $request->student_id;
			$trakingStudentRoutes->student_start_photo = '';
			$trakingStudentRoutes->student_end_photo = $uploadFile;;
			$trakingStudentRoutes->end_datetime = date('Y-m-d h:i:s');
			$trakingStudentRoutes->student_address = $request->address;
			...
			$getParentData = DB::table('sm_students')->select('sm_parents.user_id as parent_user_id')->join('sm_parents', 'sm_parents.id', '=', 'sm_students.parent_id')->where('sm_students.id', trim($request->student_id))->first();
			$getDriverDetails = DB::table('sm_staffs')->select('users.id', 'users.full_name','users.email','users.device_token','users.device_id')
			->join('users', 'users.id', '=', 'sm_staffs.user_id')
			->where('sm_staffs.id', $trakings->driver_id)
			->first();
			$parents = User::where('id', '=', $getParentData->parent_user_id)->select('id', 'full_name','email','device_token','device_id')->get();
			if(!empty($parents)) {
				foreach($parents as $adminValue) {
					if($adminValue->device_id) {
						$body['title'] = 'Driver '.$getDriverDetails->full_name.' has reached your home and drop your child '.date('Y-m-d H:i:s');
						$body['type'] = 'parent';
						$body['message'] = 'Driver '.$getDriverDetails->full_name.' has reached your home and drop your child at'.date('Y-m-d H:i:s');
						$title = 'Driver '.$getDriverDetails->full_name.' has reached your home and drop your child '.date('Y-m-d H:i:s');
						$device_id = $adminValue->device_token;
						$response = PushNotification('Android',true,$device_id,$title,$body);
					}
					...
					$notification->academic_id = SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
					$notification->date = date('Y-m-d');
					$notification->message = 'Driver '.$getDriverDetails->full_name.' has reached your home and drop your child at'.date('Y-m-d H:i:s');
					$notification->save();
			...
			$student_list = SmStudent::find($request->student_id);
			$student_list->current_address = $request->address;
			$student_list->current_address_lat = $request->latitute;
			$student_list->current_address_long = $request->longitute;
			$student_list->save();
			...
			if (ApiBaseMethod::checkUrl($request->fullUrl())) {
				return ApiBaseMethod::sendResponse($trakingStudentRoutes, null);
			}
		}
			...
			try {
				...
				if ($user != "") {
					if (Hash::check($request->password, $user->password)) {
						...
						return ApiBaseMethod::sendError('These credentials do not match our records. 1');
					}
					} else {
					return ApiBaseMethod::sendError('These credentials do not match our records. 2');
				}
				} catch (\Exception $e) {
				return ApiBaseMethod::sendError('Error.', $e->getMessage());
			}
			...
			try {
				...
				if (Auth()->user()->role_id == 1) {
					...
					if($request->content_type) {
						$uploadContents = SmTeacherUploadContent::where('content_type', $request->content_type)->where('academic_id', SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->get();
						} else {
						$uploadContents = SmTeacherUploadContent::where('academic_id', SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->get();
					}
				}
				...
				if (ApiBaseMethod::checkUrl($request->fullUrl())) {
					$data = [];
					$data['contentTypes'] = ['as' => 'assignment','st' => 'Study material','sy' => 'syllabus','ot' => 'Others Download'];
		...
		public function apply_leave_index(Request $request)
		{
			...
			try {
				...
				->get();
				if ($user) {
					$my_leaves = SmLeaveDefine::leftjoin('sm_leave_types', 'sm_leave_types.id', 'sm_leave_defines.type_id')->where('sm_leave_defines.role_id', 2)->whereIn('sm_leave_defines.user_id', $std_id->pluck('user_id'))->where('sm_leave_defines.school_id', Auth::user()->school_id)->select('sm_leave_defines.*','sm_leave_types.type')->get();
					...
					$leave_types = SmLeaveDefine::leftjoin('sm_leave_types', 'sm_leave_types.id', 'sm_leave_defines.type_id')->where('sm_leave_defines.role_id', 2)->whereIn('sm_leave_defines.user_id', $std_id->pluck('user_id'))->where('sm_leave_defines.active_status', 1)->where('sm_leave_defines.academic_id', getAcademicId())->where('sm_leave_defines.school_id', Auth::user()->school_id)->select('sm_leave_defines.*','sm_leave_types.type')->get();
					} else {
					$my_leaves = SmLeaveDefine::leftjoin('sm_leave_types', 'sm_leave_types.id', 'sm_leave_defines.type_id')->where('sm_leave_defines.role_id', $request->role_id)->select('sm_leave_defines.*','sm_leave_types.type')->get();
					$apply_leaves = SmLeaveRequest::where('role_id', $request->role_id)->where('active_status', 1)->get();
					$leave_types = SmLeaveDefine::leftjoin('sm_leave_types', 'sm_leave_types.id', 'sm_leave_defines.type_id')->where('sm_leave_defines.role_id', $request->role_id)->where('sm_leave_defines.active_status', 1)->select('sm_leave_defines.*','sm_leave_types.type')->get();
				}
				
				if (ApiBaseMethod::checkUrl($request->fullUrl())) {
					$data = [];
					$data['my_available_leaves'] = $my_leaves->toArray();
					$data['apply_leaves_list'] = $apply_leaves->toArray();
					$data['leave_types'] = $leave_types->toArray();
					return ApiBaseMethod::sendResponse($data, null);
				}
		...
		*/
		...
		public function classRoutineApi(Request $request, $id)
		{
			...
			$student_detail = SmStudent::where('id', $id)->first();
			$studentRecord = StudentRecord::where('student_id', $student_detail->id)->first();
			...
			if (ApiBaseMethod::checkUrl($request->fullUrl())) {
				...
				foreach ($weekenD as $row) {
					...
					->select('sm_class_routine_updates.start_time', 'sm_class_routine_updates.end_time', 'sm_subjects.subject_name', 'sm_class_rooms.room_no')
		...
		public function childHomework(Request $request, $id)
		{
			...
			if (ApiBaseMethod::checkUrl($request->fullUrl())) {
				$student_detail = SmStudent::where('id', $id)->first();
				$student_detail = StudentRecord::where('student_id', $student_detail->id)->first();
				$class_id = $student_detail->class_id;
		...
		public function childAttendanceAPI(Request $request, $id)
		{
			...
			->select('attendance_type', 'attendance_date','attendance_excuse')
			->get();
		...
		public function pushNotificationAndroid(Request $request) {
			$body['title'] = 'Testing Notification Metahops test';
			$body['type'] = 'parent';
            $body['message'] = 'Welcome to Metahops Testing Successfully done';
            $title = 'New Teacher Signed up. Approve test';
			$device_id = $request->device_id; //'';
			$response = PushNotification('Android',true,$device_id,$title,$body);
			return ApiBaseMethod::sendResponse($response, null);
		}
