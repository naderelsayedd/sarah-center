...
class SmsTemplate extends Model
{
    ...
    public static function emailTempleteToBody($body, $data)
    {
		...
		$activation_link = url('account/activation' . '/' . @$data['user_email'] . '/' . @$data['random']);
		
		

        $body = str_replace('http://[activation_link]', @$activation_link, $body);
        $body = str_replace('https://[activation_link]', @$activation_link, $body);
        $body = str_replace('//[activation_link]', @$activation_link, $body);
        $body = str_replace('[activation_link]', @$activation_link, $body);
		
		
		$login_link = url('login');
		$body = str_replace('http://[login_link]', @$login_link, $body);
        $body = str_replace('https://[login_link]', @$login_link, $body);
        $body = str_replace('//[login_link]', @$login_link, $body);
        $body = str_replace('[login_link]', @$login_link, $body);
		
		if($data['not_approved_reason']) {
		$body = str_replace('[not_approved_reason]', @$data['not_approved_reason'], $body);
		}
