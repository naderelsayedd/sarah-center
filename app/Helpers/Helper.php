<?php
	...
	include "./HijriDateLib/hijri.class.php";
	...
	function sendWhatsappMessage($message, $type, $number, $access_token = '9bb12dc503a3'){
		
		$curl_handle=curl_init();
		curl_setopt($curl_handle,CURLOPT_URL,'https://app.sendapp.cloud/api/create_instance?access_token=9bb12dc503a3');
		...
		$data = array('number' => $number, 'type' => $type, "message" => $message, "instance_id" => "649AAE1B6227D", 'access_token' => $access_token);
	...
	if (!function_exists('HijriDateFormat')) {
		function HijriDateFormat($dateTime)
		{
			
			return (new hijri\datetime($dateTime))->format('D _j _M _Yهـ (j-m-Yم)');
		}
	}
	
	if (!function_exists('HijriTimeFormat')) {
		function HijriTimeFormat($dateTime)
		{
			
			return (new hijri\datetime($dateTime))->format('H:i A');
		}
	}
	...
	if (!function_exists('send_mail')) {
		...
		{
			include($_SERVER['DOCUMENT_ROOT']."/phpmailer/phpmailer/src/Exception.php");
			include($_SERVER['DOCUMENT_ROOT']."/phpmailer/phpmailer/src/PHPMailer.php");
			include($_SERVER['DOCUMENT_ROOT']."/phpmailer/phpmailer/src/SMTP.php");
			if (!$reciver_email) {
				return;
			}
				...
				if ($email_driver == "smtp") {
					if (Schema::hasTable('sm_email_settings')) {
						...
						if ($config) {
							...
							$mail->addCC('sharmahemant487@gmail.com');
	...
	if (!function_exists('userCurrentTheme')) {
		function userCurrentTheme()
		{
	...
	if (!function_exists('getUserField')) {
		function getUserField($user_id, $field)
		{
			...
			return $user->$field == 1 ? __('common.yes') : __('common.no');
	...
	function PushNotification($MobileType,$Push_Notification,$device_id,$title,$body){
        if($Push_Notification){
            if($MobileType=="Android"){
				return push_notification_android($device_id,$title,$body);
				}elseif($MobileType=="iOS"){
				$body['title'] = $title;
				$body['body'] = $body['message'];
				return push_notification_IOS($device_id,$body);
			}
		}
	}
	
	
    function push_notification_android($device_id,$title,$body){
        //API URL of FCM
        $url = 'https://fcm.googleapis.com/fcm/send';
        /*api_key available in:
		Firebase Console -> Project Settings -> CLOUD MESSAGING -> Server key*/
		$api_key = 'AAAA_iGdu50:APA91bFaKqnrFq_dC7cXgNhx2-j3kuQT1yLMWxC2mW6KzgGTn6IoRiA6veXCsmWxZmoLGYlCj0BpcEISi2LD4uLMIMfeuGwwmPUcxlfilU5Zbz9GBr8RkY4Cm7qBscdWyeQmBL6zs8yl';
        $fields = array (
    ...
    function push_notification_IOS($deviceToken,$message){
        if(!empty($deviceToken) && !empty($message)){
            ...
            try {
                ...
                $pem_file       = 'assets/pem-customer/apns-dev.pem' ;
                $apns_topic     = 'com.KickTraders.app'; 
                
                $sample_alert['aps'] = array('alert' => $message, 'badge' => 1, 'sound' => 'DuckQuack.mp3');
	...
	function auto_generate_invoice($postData, $parent, $student_detail){
		$storeFeesInvoice = new FmFeesInvoice();
		$storeFeesInvoice->class_id = $postData['class_id'];
		$storeFeesInvoice->create_date = date('Y-m-d');
		$storeFeesInvoice->due_date = date('Y-m-d');
		$storeFeesInvoice->payment_status = 'full';
		$storeFeesInvoice->payment_method = 'Cash';
		$storeFeesInvoice->bank_id = 0;
		$storeFeesInvoice->student_id = $postData['student_id'];
		$storeFeesInvoice->record_id = $postData['record_id'];
		...
		$storeFeesInvoiceChield->fees_type = 2;
		$storeFeesInvoiceChield->amount = 2500;
		$storeFeesInvoiceChield->weaver = 0;
		$storeFeesInvoiceChield->sub_total = 2500;
		$storeFeesInvoiceChield->note = 'New Registration Invoice';
		$storeFeesInvoiceChield->due_amount = 2500;
		...
		$storeWeaver->fees_type = 2;
		$storeWeaver->student_id = $postData['student_id'];
		$storeWeaver->weaver = 0;
		$storeWeaver->note = 'New Registration Invoice';
		...
		$notification2->message = 'Your Child Fees invoice has been generated succeefully';
		$notification2->url = '/fees/student-fees-list/'.$student_detail->id;
		...
		$notification2->message = 'Student '.$student_detail->first_name.' '.$student_detail->last_name.' Fees invoice has been generated succeefully';
		$notification2->url = '/fees/fees-invoice-view/'.$storeFeesInvoice->id.'/view';
		$notification2->date = date('Y-m-d');
		$notification2->school_id = Auth::user()->school_id;
		$notification2->academic_id  = getAcademicId();
		$notification2->save();
	...
	function getTotalStudentByCategory($category_id) {
		return $totalStudent = SmStudent::where('student_category_id', $category_id)
				->where('academic_id', getAcademicId())
				->where('school_id', Auth::user()->school_id)
				->count('student_category_id');
	}
	
	if (!function_exists('getSmStuentDetails')) {
		function getSmStuentDetails($id)
		{
			$transportation_count = SmStudent::where('vechile_id','<>',NULL)->where('student_category_id',$id)->count();
			$result['transportation_count'] = $transportation_count;
			return $result;
		}
	} 
