...
trait UserChatMethods{
    ...
    public function userSpecificConversationCollectionUnread($userId)
    {
        $conversations = Conversation::with('forwardFrom','reply','fromUser','toUser')->where(function ($query) use($userId){
            $query->where('from_id', $userId);
            $query->where('to_id', auth()->id());
        })->get();

        return $conversations->where('status', 0)->count();
    }
